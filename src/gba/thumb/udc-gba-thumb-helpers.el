...
(defun u/gba/thumb-key-pressed (symtab r key)
  "Generate code placing a nonzero value in register R if KEY is pressed.
  ...
  (let ((mask (or (alist-get key u/gba/button-masks) (error "Unknown key: %s" key))))
    (u/gba/emit!
      (u/gba/scope
        ...
        (let ((mr (u/gba/thumb-fresh-constant mask)))
          (u/gba/emit!
            `(mvn ,r ,r)
            `(and ,r ,mr)))))))

(defun u/gba/thumb-if-cond (cond then &optional else)
  "Generate a branch based on the condition flag COND.
If C is nonzero, execute the code generated by THEN,
otherwise execute the code generated by ELSE."
  (u/gba/emit!
    (u/gba/scope
      (u/gba/emit!
        `(,(intern (format "b%s" cond)) :then))
      (when else
        (funcall else))
      (u/gba/emit!
        '(b :done)
...
(defun u/gba/thumb-if (c then &optional else)
  "Generate a branch based on the value in the register C.
If C is nonzero, execute the code generated by THEN,
otherwise execute the code generated by ELSE."
  (u/gba/emit!
    (u/gba/scope
...
(defun u/gba/thumb-bg-scroll-horizontal (syms idx r)
  "Set the horizontal scroll for background IDX to the value in register R.
Lookup the control register in SYMS."
  (u/gba/thumb-set16 syms (intern (format ":reg-bg%shofs" idx)) r))
(defun u/gba/thumb-bg-scroll-vertical (syms idx r)
  "Set the vertical scroll for background IDX to the value in register R.
Lookup the control register in SYMS."
  (u/gba/thumb-set16 syms (intern (format ":reg-bg%svofs" idx)) r))
